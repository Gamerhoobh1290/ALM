...
class MockResponse:
    ...
    def read(self):
        ...
        ckpt_path = str(ckpt).replace('\\', '\\\\')
        return b'{"choices": [{"message": {"content": "{\\"hypothesis\\": \\"test\\", \\"parent_checkpoint\\": \\"' + ckpt_path.encode() + b'\\", \\"dataset\\": \\"dolly\\", \\"target_tokens\\": 100, \\"max_seconds\\": 60, \\"eval_plan\\": \\"test\\"}"}}]}'
